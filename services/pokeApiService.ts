...
import { findPokemonInWildZones } from './locationService';
...
// FIX: Implement and export the missing fetchPokemonGenerationInsights function.
export const fetchPokemonGenerationInsights = async (pokemonId: number): Promise<GenerationInsights | null> => {
    // Re-use the existing location finding logic for Kalos.
    const locations = findPokemonInWildZones(pokemonId);
    
    if (locations.length === 0) {
        return {
            availability: [{ 
                area: 'Not available in Kalos wild areas', 
                notes: 'This Pokémon might be available through other means like trading, breeding, or special events.' 
            }]
        };
    }
    
    // Map the found locations to the format expected by the GenerationInsights type.
    const availability = locations.map(loc => ({
        area: loc.zoneName,
        notes: loc.conditions || 'Standard encounter'
    }));
...
export const fetchPokemonDetails = async (pokemonNameOrId: string | number): Promise<PokemonDetailData> => {
  ...
  const flavorTextEntry = speciesData.flavor_text_entries.find(entry => entry.language.name === 'en' && (entry.version.name === 'x' || entry.version.name === 'y'));
  const flavorText = flavorTextEntry ? flavorTextEntry.flavor_text.replace(/[\n\f\r]/g, ' ') : "No flavor text available for Pokémon X & Y.";
  ...
  const levelUpMoves = pokemonData.moves
    .map(moveData => {
      const xyDetail = moveData.version_group_details.find(
        detail => detail.version_group.name === 'x-y' && detail.move_learn_method.name === 'level-up'
      );
      if (xyDetail && xyDetail.level_learned_at > 0) {
        return {
          rawName: moveData.move.name,
          name: moveData.move.name.replace(/-/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
          levelLearnedAt: xyDetail.level_learned_at,
...
export const fetchAbilityDetails = async (abilityNameOrId: string | number): Promise<AbilityDetailData> => {
  ...
  const flavorTextEntry = abilityData.flavor_text_entries.find(
    ft => ft.language.name === 'en' && (ft.version_group.name === 'x-y')
  );
  ...
  const processedData: AbilityDetailData = {
    ...
    flavorText: flavorTextEntry ? flavorTextEntry.flavor_text.replace(/\s+/g, ' ') : "No flavor text available for Pokémon X & Y.",
