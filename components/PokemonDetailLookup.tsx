...
export const PokemonDetailLookup: React.FC<PokemonDetailLookupProps> = ({ onAbilityClick, onMoveClick }) => {
    ...
    const handleSearch = useCallback(async (pokemonNameOrId: string | number) => {
        ...
        try {
            ...
            try {
                // FIX: Pass pokemon ID instead of name for better performance and to match the new function signature.
                const insights = await fetchPokemonGenerationInsights(details.id);
                setPokemon(prevDetails => prevDetails ? { ...prevDetails, generationInsights: insights } : null);
            } catch (insightsError) {
                console.warn(`Could not fetch Gen 6 insights for ${details.name}:`, insightsError);
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                ...
                                {pokemon.generationInsights === undefined ? (
                                    <div className="bg-slate-700/50 p-4 rounded-lg text-center text-slate-400 italic">Loading Kalos Info...</div>
                                ) : pokemon.generationInsights ? (
                                    <div className="bg-slate-700/50 p-4 rounded-lg">
                                        <h4 className="font-bold text-sky-300 mb-2">Availability in Kalos</h4>
                                ...
                                <div className="bg-slate-700/50 p-4 rounded-lg">
                                    <h4 className="font-bold text-sky-300 mb-2">Key Level-Up Moves (Gen 6)</h4>
