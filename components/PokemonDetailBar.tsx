...
import { findPokemonInWildZones } from '../services/locationService';
...
const ConditionBadge: React.FC<{ conditions?: string }> = ({ conditions }) => {
  ...
  return (
    <span 
        className={`ml-auto px-2 py-0.5 rounded-full text-xs font-semibold ${bgColor} ${textColor} inline-flex items-center whitespace-nowrap`}
        title={conditions} 
    >
...
export const PokemonDetailBarComponent: React.FC<PokemonDetailBarProps> = ({ 
  ...
  useEffect(() => {
    // Reset shiny view when pokemon changes
    setShowShiny(false);
  }, [pokemonData.id]);
  ...
  const locations = findPokemonInWildZones(pokemonData.id);
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-x-4 gap-y-2">
        ...
        <div className="flex flex-col items-center text-center col-span-1 md:col-span-1 space-y-2">
            ...
            <h2 className="text-xl md:text-2xl font-bold text-sky-300">{pokemonData.name}</h2>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-center gap-2 w-full pt-1">
                ...
                className={`w-full sm:w-auto flex-grow sm:flex-grow-0 py-1.5 px-3 text-xs rounded-md font-semibold transition-colors ${
                    isCaught ? 'bg-sky-500 hover:bg-sky-600 text-white' : 'bg-slate-600 hover:bg-slate-500 text-slate-200'
                }`}
                >
                ...
                <button
                    ...
                    className="w-full sm:w-auto flex-grow sm:flex-grow-0 py-1.5 px-3 text-xs rounded-md font-semibold bg-blue-600 hover:bg-blue-700 text-white transition-colors"
                >
                    Add to Party
                </button>
                 <button
                    onClick={() => onAddToHuntingList(pokemonData.id, pokemonData.name, 'General Hunt')}
                    className="w-full sm:w-auto flex-grow sm:flex-grow-0 py-1.5 px-3 text-xs rounded-md font-semibold bg-cyan-600 hover:bg-cyan-700 text-white transition-colors"
                >
                    Add to Hunt
                </button>
            </div>
        ...
        {/* Column 4: Key Moves */}
        <div className="col-span-1 md:col-span-1">
            <h3 className="text-base md:text-lg font-semibold mb-1 text-sky-300">Key Level-Up Moves <span className="text-xs text-slate-400">(Gen 6)</span></h3>
            {pokemonData.moves.length > 0 ? (
                ...
                <p className="text-xs md:text-sm text-slate-400 italic">No specific level-up moves found for Gen 6.</p>
            )}
        </div>
    </div>
        
    <div className="mt-4 pt-3 border-t border-slate-700">
        <h3 className="text-base md:text-lg font-semibold mb-2 text-sky-300">Locations</h3>
        {locations.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {locations.map(loc => (
                    <div key={loc.zoneName} className="text-sm text-slate-300 bg-slate-700/40 p-2 rounded-md flex justify-between items-center">
                        <strong className="text-slate-100">{loc.zoneName}</strong>
                        <ConditionBadge conditions={loc.conditions} />
                    </div>
                ))}
            </div>
        ) : (
            <p className="text-sm text-slate-400 italic">Not found in any known wild zones.</p>
        )}
    </div>
    </div>
  );
