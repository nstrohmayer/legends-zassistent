...
const PokemonTeamCard: React.FC<PokemonTeamCardProps> = ({
  ...
  const smogonUrl = `https://www.smogon.com/dex/xy/pokemon/${member.species.toLowerCase().replace(/\s+/g, '-')}/`;
  ...
  return (
    <div className="bg-slate-800/60 p-3 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 relative group backdrop-blur-sm border border-slate-700 text-sm">
      <button
      ...
      <div className="flex items-start space-x-3">
        {spriteUrl ? (
          <img 
            ...
            className="w-16 h-16 pixelated-sprite bg-slate-700/50 rounded-md mt-1" 
            onError={(e) => (e.currentTarget.style.display = 'none')} // Hide if sprite fails to load
          />
        ) : (
          <div className="w-16 h-16 bg-slate-700/50 rounded-md flex items-center justify-center mt-1">
        ...
        <div className="flex-1 space-y-1.5">
          <input
            ...
            className="w-full bg-slate-900/70 text-slate-100 placeholder-slate-400 px-2 py-1 rounded-md text-base font-semibold border border-slate-700 focus:ring-1 focus:ring-sky-500 focus:border-sky-500 outline-none"
          />
          <div className="flex items-center space-x-2">
            ...
            <input
              ...
              min="1"
              max="100"
              className="w-16 bg-slate-900/70 text-slate-100 px-2 py-0.5 rounded-md text-xs border border-slate-700 focus:ring-1 focus:ring-sky-500 focus:border-sky-500 outline-none"
            />
             <button 
      ...
      <div className="mt-2 space-y-1">
        <input
            ...
            className="w-full bg-slate-900/70 text-slate-100 placeholder-slate-400 px-2 py-1 rounded-md text-xs border border-slate-700 focus:ring-1 focus:ring-sky-500 focus:border-sky-500 outline-none"
        />
...
export const TeamManager: React.FC<TeamManagerProps> = ({ 
    ...
    const teamWeaknesses = useMemo<Record<string, { count: number; effectiveness: number[] }>>(() => {
    ...
    <div className="space-y-6">
        <div className="flex justify-end -mb-2">
            <button
                ...
                className="px-4 py-2 text-sm font-semibold rounded-md bg-slate-700 hover:bg-slate-600 text-white transition-colors"
            >
      ...
      {isEditMode ? (
        <div className="animate-fadeIn space-y-4">
            <div>
                ...
                <textarea
                    ...
                    className="w-full p-3 bg-slate-900/60 border border-slate-700 rounded-lg text-slate-100 placeholder-slate-400 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-colors font-mono text-sm"
                    rows={18}
            ...
            <button
                ...
                className="w-full px-6 py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-md shadow-md transition-all duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
                {isLoadingImport ? (
                    <>
        ...
        <>
            ...
            {team.length > 0 && (
                    <div className="mt-6">
                        <h3 className="text-xl font-bold text-sky-300 mb-3">Team Weaknesses</h3>
                        <div className="bg-slate-800/60 p-4 rounded-lg border border-slate-700">
                            {Object.keys(teamWeaknesses).length > 0 ? (
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-4 gap-y-3">
                                    {/* FIX: Using Object.keys() for iteration to ensure proper type inference for teamWeaknesses values, avoiding issues with Object.entries() returning `unknown`. */}
                                    {Object.keys(teamWeaknesses).sort((a, b) => teamWeaknesses[b].count - teamWeaknesses[a].count).map((type) => {
                                        const data = teamWeaknesses[type];
                                        const isMajorWeakness = data.effectiveness.some(e => e >= 4);
                                        return (
                                            <div key={type} className="flex items-center gap-2 bg-slate-900/50 p-2 rounded-md">
                                                <TypeBadge type={type} />
